...
import { parseStageKey } from "@/pipeline/orchestrator";
...
export async function POST(
  ...
  const stepResult = getStepResult(id, stage);
  if (!stepResult) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        saveStepResult(id, stage, revisedJson);
        archiveActiveAnnotations(id, stage);

        const { baseStage } = parseStageKey(stage);
        const stageIdx = STAGE_ORDER.indexOf(baseStage);
        const nextStage = stageIdx >= 0 && stageIdx < STAGE_ORDER.length - 1
