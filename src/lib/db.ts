...
export type InputMode = "article" | "book";
...
function getDb(): Database.Database {
  ...
  db.exec(`
    CREATE TABLE IF NOT EXISTS projects (
      ...
      input_mode TEXT NOT NULL DEFAULT 'article',
      sections_json TEXT,
  ...
  try {
    db.exec(`ALTER TABLE projects ADD COLUMN input_mode TEXT NOT NULL DEFAULT 'article'`);
  } catch { /* column already exists */ }
  try {
    db.exec(`ALTER TABLE projects ADD COLUMN sections_json TEXT`);
  } catch { /* column already exists */ }
...
export function createProject(
  ...
  const firstStage = inputMode === "book" ? "section_splitting" : "text_analysis";
  db.prepare(`
    INSERT INTO projects (id, name, article_text, input_mode, current_stage, status, created_at, updated_at)
    VALUES (?, ?, ?, ?, ?, 'idle', ?, ?)
  `).run(id, name, articleText, inputMode, firstStage, now, now);
  return getProject(id)!;
}

export function getSections(projectId: string): BookSection[] {
  const project = getProject(projectId);
  if (!project?.sections_json) return [];
  return JSON.parse(project.sections_json) as BookSection[];
}

export function updateSections(projectId: string, sections: BookSection[]): void {
  const db = getDb();
  const now = new Date().toISOString();
  db.prepare(
    "UPDATE projects SET sections_json = ?, updated_at = ? WHERE id = ?"
  ).run(JSON.stringify(sections), now, projectId);
}
...
export function updateProject(
  id: string,
  fields: Partial<Pick<ProjectRow, "name" | "current_stage" | "status" | "output_path" | "error" | "sections_json">>
): void {
  const db = getDb();
  const sets: string[] = [];
...
export function saveStepResult(projectId: string, stage: string, resultJson: string): void {
  const db = getDb();
  const now = new Date().toISOString();
  db.prepare(`
...
export function getStepResult(projectId: string, stage: string): StepResultRow | undefined {
  const db = getDb();
  return db.prepare(
...
export function clearStepsFrom(projectId: string, stage: string): void {
  const db = getDb();
  const baseStage = stage.split(":")[0] as PipelineStage;
  const idx = stageIndex(baseStage);
  if (idx < 0) return;

  // For book mode, also clear per-section composite keys (e.g. game_design:0)
  const stagesToClear = STAGE_ORDER.slice(idx);
  const likePatterns = stagesToClear.map(s => `stage = '${s}' OR stage LIKE '${s}:%'`).join(" OR ");
  db.prepare(
    `DELETE FROM step_results WHERE project_id = ? AND (${likePatterns})`
  ).run(projectId);
...
export function clearDownstreamResults(projectId: string, afterStage: string): void {
  const db = getDb();
  const baseStage = afterStage.split(":")[0] as PipelineStage;
  const idx = stageIndex(baseStage);
